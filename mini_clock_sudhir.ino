...
#include "themes.h"                      //add Note value and duration 
...
// Milis used instead of delay funtion
...
Button buttonC = Button(4, BUTTON_PULLUP);      // Setup button A (using button library)
...
int setBright(){
  ...
  int new_brightness = map(analogRead(LIGHT), 0, 1023, 0, 15);

if (new_brightness > brightness+1 || new_brightness < brightness-1)
{
  //we have to init all devices in a loop
...
void small() {
  ...
  if (buttonC.uniquePress()) { toggleAlmState();  delay(1000); return; }
...
//  get_time(); //get the time from the clock chip
  ...
  while (run_mode()) {
    ...
    if (hours == 0) { hours = 12; }
       ...
       else if (mins >= 10 && mins <= 14){ strcpy (str_a, "Two");     strcpy (str_b, "AFTER"); strcpy (str_c, ""); }
       else if (mins >= 15 && mins <= 19){ strcpy (str_a, "QUARTER");  strcpy (str_b, "AFTER"); strcpy (str_c, ""); }
       else if (mins >= 20 && mins <= 24){ strcpy (str_a, "Two");     strcpy (str_b, "BEFORE");  strcpy (str_c, "HALF"); half = 1; }      
       else if (mins >= 25 && mins <= 29){ strcpy (str_a, "Five");     strcpy (str_b, "BEFORE");  strcpy (str_c, "HALF"); half = 1; }      
       else if (mins >= 30 && mins <= 34){ strcpy (str_a, "");         strcpy (str_b,    "");  strcpy (str_c, "HALF"); half = 1; }      
       else if (mins >= 35 && mins <= 39){ strcpy (str_a, "Five");     strcpy (str_b, "AFTER"); strcpy (str_c, "HALF"); half = 1; }      
       else if (mins >= 40 && mins <= 44){ strcpy (str_a, "Two");     strcpy (str_b, "AFTER"); strcpy (str_c, "HALF"); half = 1; }       
       else if (mins >= 45 && mins <= 49){ strcpy (str_a, "QUARTER");  strcpy (str_b, "BEFORE");  strcpy (str_c, ""); half = 1; }     
       else if (mins >= 50 && mins <= 54){ strcpy (str_a, "Two");     strcpy (str_b, "BEFORE");  strcpy (str_c, ""); half = 1; }       
       else if (mins >= 55 && mins <= 59){ strcpy (str_a, "Five");     strcpy (str_b, "BEFORE");  strcpy (str_c, ""); half = 1; } 

      int wordHour = hours + half;
      ...
      else if ( wordHour == 11) { strcpy (str_d, "Ele"); } 
    ...
    //print line b, if not empty / "AFTER"/"BEFORE"/""
    char bl = str_b[0];
    if(bl>1){
    len = 0;
    while (str_b[len]) {
    ...
    //print line c, if not empty / "HALF"
    char cl = str_c[0];
    if(cl>1){
    len = 0;
    while (str_c[len]) {
...
////////////////////////////////////////////////////////////////////////////////////////
//Display Alarm written
void disp_alm()
{
    time_now = millis();
    if (buttonA.uniquePress() || buttonB.uniquePress() || buttonC.uniquePress()) 
    {
      alarm_mode = 0;
      chks=1;
      return;
...
void toggleAlmState()
            {     //if alarm mode is on, turn it off
                    if (alarm_mode)
                    {
                      ...
                      char almoN[8] = "ALM:OFF";
                      int len=7;  // length of almoN               
                      byte offset_top = (31 - ((len - 1) * 4)) / 2; 
                      byte i = 0;
                      while (almoN[i])
                      {
                        puttinychar((i * 4) + offset_top, 1, almoN[i]);
                        i++;
                      }
                      //hold display but check for button presses
                      int counter = 1000;
                    ...
                    else 
                    {
                      ...
                      char almoFF[8] = " ALM:ON";
                      int len=7;  // length of almoN               
                      byte offset_top = (31 - ((len - 1) * 4)) / 2; 
                      byte i = 0;
                      while (almoFF[i])
                      {
                        puttinychar((i * 4) + offset_top, 1, almoFF[i]);
                        i++;
                      }
                      //hold display but check for button presses
                      int counter = 1000;
...
void set_al() {
  ...
  //Set function - we pass in: which 'set' message to show at top, current value, reset value, and rollover limit.
  al[1]   = set_value(1, set_hr, 0, 23);
  al[0]  = set_value(0, set_min, 0, 59);
